...
function agentVaultRulesFile() {
  return path.join(agentRulesDir(), 'obsidian-vault.md');
}
...
const DEFAULT_OBSIDIAN_VAULT_RULES = `# Obsidian Vault ??????

## ????

Hermes WebUI ??????? Obsidian ??? Markdown Vault???????????? AI ??????????????????????????Prompt?????????????????????????
...
- ????????????????????????????
- ????????????????????????????
- Prompt????? Prompt??? Prompt?UI Prompt?Agent Prompt???????
- ???????????????UI ?????????????
- ???????????? Prompt??? Prompt????????????
...
?? Markdown ????? YAML frontmatter?title?folder?type?tags?created?source?status?summary?related?????????? Obsidian ??????????????? Prompt ??? Prompt?
`;
...
function ensureMemoryStore() {
  ...
  if (!fs.existsSync(agentVaultRulesFile())) fs.writeFileSync(agentVaultRulesFile(), DEFAULT_OBSIDIAN_VAULT_RULES, 'utf8');
...
function readAgentRulesPrompt(options = {}) {
  const sections = [
    readRuleFilePrompt(agentAlwaysRulesFile(), '?????'),
    readRuleFilePrompt(agentVaultRulesFile(), 'Obsidian Vault ??????'),
  ];
  if (options.includeKnowledgeBase) {
    sections.push(readRuleFilePrompt(agentRulesFile(), '???? Markdown ????'));
  }
...
module.exports = {
  ...
  get AGENT_VAULT_RULES_FILE() { return agentVaultRulesFile(); },
  getOverview,
  listCoreFiles,
  readCoreFile,
  writeCoreFile,
